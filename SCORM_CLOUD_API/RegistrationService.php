...
class RegistrationService{
	...
	public function __construct($configuration) {
        ...
        public function CreateNewInstance ($registrationId)
        {
            $request = new ServiceRequest($this->_configuration);
            $request->setMethodParams(array('regid' => $registrationId));
            $response = $request->CallService("rustici.registration.createNewInstance");
			
			$xml = simplexml_load_string($response);
            
			return $xml->success['instanceid'];           
        }
